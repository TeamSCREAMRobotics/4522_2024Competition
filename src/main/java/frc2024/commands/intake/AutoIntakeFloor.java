...
public class AutoIntakeFloor extends SequentialCommandGroup {
  ...
  public AutoIntakeFloor(DoubleSupplier[] translation, Swerve swerve, Intake intake, Conveyor conveyor) {
    addCommands(
  ...
  public AutoIntakeFloor(Swerve swerve, Intake intake, Conveyor conveyor) {
    addCommands(
      new FaceVisionTarget(
        swerve, 
        new DoubleSupplier[]{() -> 0, () -> 3}, 
        ...
        Limelight.INTAKE)
          .alongWith(
            intake.outputCommand(IntakeConstants.INTAKE_SPEED)
              .onlyIf(() -> Vision.getTY(Limelight.INTAKE) < IntakeConstants.AUTO_INTAKE_TY_THRESHOLD && !conveyor.hasPiece())
          )
    );
  }
}
