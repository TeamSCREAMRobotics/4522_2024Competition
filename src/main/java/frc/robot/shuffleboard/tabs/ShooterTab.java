...
public class ShooterTab extends ShuffleboardTabBase{
    ...
    public void createEntries() {
        ...
        m_motorVelocity = createNumberEntry("Motor Velocity", 0, new EntryProperties(2, 0, 2, 1));
        m_wheelVelocity = createNumberEntry("Wheel Velocity", 0, new EntryProperties(2, 1, 2, 1));

        if(ShuffleboardConstants.UPDATE_SHOOTER){
            m_dutyCycle = createNumberEntry("Shooter Duty Cycle", 0.0, new EntryProperties(0, 0, 2, 1));
            m_velocity = createNumberEntry("Shooter Velocity", 0.0, new EntryProperties(0, 1, 2, 1));
        }
    }
    @Override
